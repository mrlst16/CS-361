...
public:
	 ...
	 *
	 * @param topic the topic to be researched
	 * @param begin.  The beginning iterator of the container of pre-required topics
	 * @param end.  The end iterator of the container of pre-required topics
	 */
	template<typename Iterator>
    ResearchPlan (const Topic topic, Iterator start, Iterator finish);

    ResearchPlan (const Topic topic, std::array<Topic, 3>::iterator start, std::array<Topic, 3>::iterator finish);
	...
	std::vector<Topic>::iterator begin(){return requirements.begin();};
	std::vector<Topic>::iterator end(){return requirements.end();};
	std::vector<Topic>::const_iterator begin() const {return requirements.begin();};
	std::vector<Topic>::const_iterator end() const {return requirements.end();};
private:
	friend std::ostream& operator<< (std::ostream& out, const ResearchPlan& plan);
	std::vector<Topic> requirements;
};
