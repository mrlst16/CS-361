        ...
        const Topic& requiredTopic)
{
	...
	cout << "Memory after findOrAdd on Encyclopedia::addPlanRequirement " << MemoryChecked::getCurrentCount() << endl;
	ResearchPlan& plan1 = findOrAdd(topic1.name);
	cout << "Memory after ResearchPlan& plan1 = findOrAdd(topic1.name) on Encyclopedia::addPlanRequirement " << MemoryChecked::getCurrentCount() << endl;
	plan1.addRequirement(requiredTopic);
    cout << "Memory after plan1.addRequirement on Encyclopedia::addPlanRequirement " << MemoryChecked::getCurrentCount() << endl;
}
